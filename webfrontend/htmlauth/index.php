...
$metaFile  = "$scriptDir/scripthub.json";
...
function getLastRunTimeFromCustomLog($scriptName, $isRunning) {
    ...
    if (preg_match('/^(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})\s+(=== Started|--- Stopped|--- Finished)/', $output, $m)) {
        $when = $m[1];
        $what = (strpos($m[2], 'Started') !== false) ? "Started" : (strpos($m[2], 'Stopped') !== false ? "Stopped" : "Finished");
        return "$when ($what)";
    }
    return "Unknown";
}
...
function script_interpreter_cmd($scriptPath) {
    ...
    if ($ext === 'php') {
        ...
        return $py.' '.escapeshellarg($scriptPath);
    } elseif ($ext === 'sh') {
        $bash = sh_which('bash', '/bin/bash');
        return $bash.' '.escapeshellarg($scriptPath);
    } else {
        return escapeshellarg($scriptPath);
    }
}
/** Build /etc/cron.d/scripthub content from metadata */
function build_cron_file($meta, $scriptDir, $logfile) {
    $lines = [];
    ...
    foreach ($meta as $script => $cfg) {
        ...
        $cron = trim($cfg['cron_expr'] ?? '');
        if ($cron === '') continue;

        $scriptPath = $scriptDir . '/' . $script;
        if (!is_file($scriptPath)) continue;

        $cmd    = script_interpreter_cmd($scriptPath);
        $lock   = '/var/lock/scripthub.' . preg_replace('/[^A-Za-z0-9_.-]/', '_', $script) . '.lock';
        $log    = $logfile;
        $user   = 'loxberry';

        $bashBlock = "printf \"%s === Started (cron) $script ===\\n\" \"\$(date '+%F %T')\" >> $log; "
                   . "$cmd >> $log 2>&1; rc=\$?; "
                   . "printf \"%s --- Finished (cron) $script ---\\n\" \"\$(date '+%F %T')\" >> $log; "
                   . "exit \$rc";
        $full = '/usr/bin/flock -n ' . $lock . ' -c ' . escapeshellarg('/bin/bash -lc ' . escapeshellarg($bashBlock));

        $lines[] = "$cron $user $full";
        $lines[] = '';
    }

    return implode("\n", $lines) . "\n";
}
/** Apply cron file (write /etc/cron.d/scripthub and reload cron), or save preview */
function apply_cron_from_meta(&$meta, $scriptDir, $logfile) {
    $content = build_cron_file($meta, $scriptDir, $logfile);

    // always save preview for diagnostics
    @file_put_contents($scriptDir . '/scripthub.cron.preview', $content);

    $target = '/etc/cron.d/scripthub';

    // direct write
    $directOK = @file_put_contents($target, $content) !== false;
    if ($directOK) {
        @chmod($target, 0644);
    ...
    return [false, "Could not write $target. Preview saved at ".$scriptDir.'/scripthub.cron.preview'];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'cron_save' && $script) {
        ...
        list($ok, $msg) = apply_cron_from_meta($meta, $scriptDir, $logfile);
    ...
    <td colspan="7" class="editor-wrap">
        ...
        <form method="post" style="margin-top:8px;">
            ...
            <div style="margin-top:12px; display:flex; gap:8px; flex-wrap:wrap;">
                ...
                <button type="button" class="btn btn-cancel" onclick="toggleSchedule('<?= $rid ?>')">Close</button>
            </div>
        </form>
